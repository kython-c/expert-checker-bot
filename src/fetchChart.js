...
const fetchChart = async (vk) => {
  ...
  const experts_count = grpInfo[0].members_count; // а вот и количество участников

  const token = await fetchToken();

  const params = { // Параметры для запроса к апи экспертов
    access_token: token,
    chart_type: 'total',
    limit: experts_count,
    v: '5.124'
  ...
  const chart = await got('https://api.vk.com/method/experts.getChart', { // Сам запрос
    searchParams: params
  });

  const parsedChart = JSON.parse(chart.body); // Парсим полученную строку в JSON
